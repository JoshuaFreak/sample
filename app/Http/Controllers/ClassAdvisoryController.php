...
class ClassAdvisoryController extends TeachersPortalMainController {   
    /**
    ...
    */
    public function index()
    {
        ...
        $pace_list = Pace::orderBy('pace.pace_name')->get();
        $student_pace_list = array();

    
        return view('teachers_portal/class_advisory.index', compact('classification_list', 'classification_level_list', 'term_list', 'action_taken_list', 'grading_period_list', 'groupby_grading_period_list','subject_list','student_pace_list','pace_list'));
    }
    /**
    ...
    public function data()
    {
      ...
      $term_id = \Input::get('term_id');
        ...
        {
            $section_adviser_list = SectionAdviser::join('section_monitor','section_adviser.section_monitor_id','=','section_monitor.id')
                ...
                ->select(array('section_adviser.id','section_monitor.classification_level_id','section_monitor.classification_id','section_monitor.term_id','classification.classification_name','classification_level.level','section.section_name','term.term_name', 'section_monitor.section_id'))
        ...
        // elseif($classification_level_id != "" && $classification_level_id != null) 
        // {
        //     $section_adviser_list = SectionAdviser::join('classification','section_monitor.classification_id','=','classification.id')
        //         ->join('classification_level','section_monitor.classification_level_id','=','classification_level.id')
        //         ->join('term','section_monitor.term_id','=','term.id')
        //         ->join('section','section_monitor.section_id','=','section.id')
    ...
    public function StudentListdata()
    {
        ...
        $section_adviser_student_list = SectionAdviserStudent::join('student','section_adviser_student.student_id','=','student.id')
            ...
            ->orderBy('person.last_name', 'ASC')
            ->groupBy('section_adviser_student.student_id');
        ...
        return Datatables::of($section_adviser_student_list)
                ...
                ->add_column('actions','
                      ...
                      <img class="button" src="{{{ asset("assets/site/images/teachers_portal/add_pace.png") }}}" data-student_id="{{{$student_id}}}" data-student_no="{{{$student_no}}}" data-last_name="{{{$last_name}}}" data-first_name="{{{$first_name}}}" data-middle_name="{{{$middle_name}}}" data-level="{{{$level}}}" data-section_name="{{{$section_name}}}" data-term_name="{{{$term_name}}}" data-term_id="{{{$term_id}}}" data-classification_name="{{{$classification_name}}}" data-classification_id="{{{$classification_id}}}" data-classification_level_id="{{{$classification_level_id}}}" data-toggle="modal" data-target="#studentpacemodal">')
    ...
    public function dataJsonStudentAttendance(){
      ...
      foreach ($attendance_list as $attendance) {
        ...
        $student_attendance_list = StudentAttendance::join('student','student_attendance.student_id','=','student.id')
          ...
          ->groupBy('student_attendance.student_id')->get();
        $row_no = 1;
        $column_no = 1;
        foreach ($student_attendance_list as $student_attendance) {
