...
class GradeSlipController extends RegistrarMainController {
   ...
   public function index()
    {
        ...
        // Show the page
        return view('registrar_report/grade_slip.index',compact('classification_list', 'classification_level_list', 'semester_level_list', 'term_list'));
    }
    ...
    public function getDetail()
    {
      ...
      return view('registrar_report/grade_slip/detail', 
        compact(
            'enrollment',
            'student',
            'term',
    ...
    public function GradeSlipreport()
    {
      ...
      // $class_id = TEClass::join('grade', 'class.id', '=', 'grade.class_id')
      ...
      $class_id = Grade::join('class', 'grade.class_id', '=', 'class.id')
                      ->join('classification_level', 'class.classification_level_id', '=', 'classification_level.id')
                      ->where('grade.student_id', '=', $student_id)
                      ->where('class.classification_level_id', '=', $classification_level_id)
                      ->where('class.section_id', '=', $section_id)
                      ->select(['class.id', 'grade.id as grade_id'])->get();

                        // echo $curriculum_subject_list;
                        // exit();
      ...
      $pdf = \PDF::loadView('registrar_report/grade_slip/grade_slip_report', array('logo'=>$logo,'classification_id'=>$classification_id,'enrollment'=>$enrollment,'student'=>$student,'term'=>$term,'classification_level'=>$classification_level,'student_curriculum'=>$student_curriculum,'curriculum_subject_list'=>$curriculum_subject_list))->setOrientation('landscape');
