        ...
        count++;
          ...
          "bProcessing" : true,
          "bServerSide" : true,
          "sAjaxSource" : "../../teachers_portal/class_monitoring/student_document_list_data",
                ...
                $('#studentdocumentlisttable tbody').on('click', 'p.details-control', function () {
                    ...
                    else 
                    {
                        ...
                        $('div.slider', row.child()).slideDown();
                          ...
                          $.ajax({
                          url: "../../teachers_portal/class_monitoring/CreateFolderdataJson",
                          data: {
                                  'folder_id': 1,
                                  'student_id': data.id,
                                },
                          ...
                          $.ajax({
                          url: "../../teachers_portal/class_monitoring/StudentDocumentJson",
                          data: {
                                  'student_id': data.id,
                                },
                          async:false,
                                ...
                                $.each( folder_list, function( key, folder ) {
                                      $("#student_document_"+data.id).append('<div class="col-md-3" id="student_file_'+folder.id+'"><b>'+folder["folder_name"]+'</b>'
                                           +'</div>');
                                        $.ajax({
                                        url: "../../teachers_portal/class_monitoring/fileDataJson",
                                        data: {
                                                'student_id': data.id,
                                                'folder_id': folder.id,
                                              },
                                                    ...
                                                    $("[data-toggle=confirmation]").confirmation({container:"body",btnOkClass:"btn btn-sm btn-success",btnCancelClass:"btn btn-sm btn-danger",
                                                            onConfirm:function(event, element) {
                                                             var id = $(this).data('id');
                                                                ...
                                                                $.ajax({
                                                                    url:"../../teachers_portal/class_monitoring/removeFile",
                                                                    type:'post',
                                                                    data:{  
                                                                            'id' : file.id,
