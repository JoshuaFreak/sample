...
{{-- Web site Title --}}
@section('title')
{{{ Lang::get("students_portal.schedule") }}} :: @parent
@stop
{{-- Content --}}
@section('content')
...
<div id="page-wrapper">
  <div class="row">
    <div class="page-header"><br>
      <h2>{{{ Lang::get("students_portal.schedule") }}}</h2>
    </div>
    ...
    <div class="col-md-12">
      ...
      <div class="col-md-3"><br/>
        <button type="button" class="btn btn-sm btn-primary" id="loadCOR">
    ...
    <div class="col-md-12" id="CorContainer"></div>
  </div>
</div>
@stop
...
<script type="text/javascript">
    ...
    $("#no_data").show();
    $("#loadCOR").click(function()
    {
        var Section = $(this).find(':selected').data('section');
        loadCor($("#classification_level_id").find(':selected').data('student'), $("#classification_level_id").find(':selected').data('section'),$("#classification_level_id").find(':selected').data('classification'),$("#classification_level_id").val(),$("#classification_level_id").find(':selected').data('term'),$("#classification_level_id").find(':selected').data('curriculum'));
        $("#no_data").hide();
    });

    function loadCor(studentId, SectionId, ClassificationId, ClassificationLevelId, TermId, CurriculumId)
    {
      $.ajax(
          {
            url: "{{{ URL::to('students_portal/schedule/detail') }}}",
            data: { 
